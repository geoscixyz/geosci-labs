...
for jj = 2 : length(x)
    ...
    % Compute vector from last point to the next
    r1 = [x(jj) - x(jj-1) y(jj) - y(jj-1)];
    
    % Compute vector from last point to median point
    r2 = [x(jj-1) - xm y(jj-1) - ym];
    
    % Compute vector from next to first
    r3 = [x(jj) - xs y(jj) - ys];
    
    % Compute vector from first to median
    r4 = [xs - xm ys - ym];
    ...
    ang1 = abs( r1/norm(r1) * r3'/norm(r3) );
    ang2 = abs( r2/norm(r2) * r4'/norm(r4) );
