...
class definePrism():
    """
        ...
        Earth's field:
            - Einc, Edec : inclination and declination of Earth's mag field
    ...
    susc,  pinc, pdec = 1., 0., 0.
    Binc, Bdec, Bigrf = 90., 0., 1e5
    Q, rinc, rdec = 0., 0., 0.
    ...
    def Mind(self):
        ...
        Mind = self.susc*self.Higrf*MAG.dipazm_2_xyz(self.Binc - self.pinc,
                                                     self.Bdec - self.pdec)
    ...
    @property
    def Mrem(self):
        Mrem = self.Q*self.susc*self.Higrf*MAG.dipazm_2_xyz(self.rinc - self.pinc,
                                                            self.rdec - self.pdec)
...
def getField(prism, rxLoc, Mcomp, Bcomp):

    rot = Utils.mkvc(MAG.dipazm_2_xyz(prism.pinc, prism.pdec))

    rxLoc = Utils.rotatePointsFromNormals(rxLoc, rot, np.r_[0., 1., 0.],
                                          np.r_[prism.xc, prism.yc, prism.zc])

    G = MAG.Intrgl_Fwr_Op(prism.xn, prism.yn, prism.zn, rxLoc)
    nD = np.shape(rxLoc)[0]

    if (Mcomp is 'induced') or (Mcomp is 'total'):

        b = G.dot(prism.Mind)

        if Bcomp is 'bx': fieldi = Utils.mkvc(b[:nD])
        if Bcomp is 'by': fieldi = Utils.mkvc(b[nD:2*nD])
        if Bcomp is 'bz': fieldi = Utils.mkvc(b[2*nD:])
    ...
    if (Mcomp is 'remanent') or (Mcomp is 'total'):

        b = G.dot(prism.Mrem)

        if Bcomp is 'bx': fieldr = Utils.mkvc(b[:nD])
        if Bcomp is 'by': fieldr = Utils.mkvc(b[nD:2*nD])
        if Bcomp is 'bz': fieldr = Utils.mkvc(b[2*nD:])
