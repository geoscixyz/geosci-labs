...
def drawfunction(delta_rho,z1,z2,b,beta,stationSpacing,B):
	...
	if B:
		plotdata.append([copy.deepcopy(Y[:,0]),copy.deepcopy(respNS),colorList[index],showText,textLocation])
		index+=1
...
def printDike(axeToDraw,z1,z2,b,beta,x):
	...
	if beta!=0:
		axeToDraw.plot([x1,x2], [x1/np.tan(beta),x2/np.tan(beta)], color='black', linewidth=1.0)
		axeToDraw.plot([x3,x4], [(x3-b)/np.tan(beta),(x4-b)/np.tan(beta)], color='black', linewidth=1.0)
	else:
		axeToDraw.plot([x1,x2], [-z1,-z2], color='black', linewidth=1.0)
		axeToDraw.plot([x3,x4], [-z1,-z2], color='black', linewidth=1.0)
	axeToDraw.plot([0,-z1*float(np.tan(beta))], [0,-z1], ':', color='black', linewidth=1.0)
...
def datagenerator(delta_rho,z1,z2,b,beta,stationSpacing):
	...
	for each in range(npts):
		gravity_change.append(calculategravity(
			delta_rho,
			z1,
			z2,
			b,
...
def interact_gravity_Dike():
	...
	s5=FloatSlider(
		description=r'$\beta$',min=-85, max=85, step=5, value=45, continuous_update=False
	)
	s6=FloatSlider(
		description='Step',min = 0.005, max=0.10, step=0.005, value=0.01,  continuous_update=False, readout_format='.3f'
	)
	b1 = ToggleButton(
		value=True,
