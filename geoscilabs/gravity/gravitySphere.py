...
def graphaDataGenerator(delta_rho,a,z,maxR,maxScala,Step):
	...
	for i in np.arange(-maxR,maxR+Step,Step):
		for j in np.arange(-maxR, maxR+Step,Step):
			scalax.append(i)
			scalay.append(j)
			color.append(calculategravity(delta_rho,a,z,pow(pow(i,2)+pow(j,2),0.5)))
...
def interact_gravity_sphere():
	Q = interactive(
		...
		a=FloatSlider(
			min=0.1, max=4.0, step=0.1, value=1.0, continuous_update=False
		),
		z=FloatSlider(
			min=0.1, max=5.0, step=0.1, value=1.0, continuous_update=False
		),
		stationSpacing=FloatSlider(
			description='Step' ,min = 0.005, max=0.1, step=0.005, value=0.01,  continuous_update=False, readout_format='.3f'
		),
		B = ToggleButton(
			value=True,
