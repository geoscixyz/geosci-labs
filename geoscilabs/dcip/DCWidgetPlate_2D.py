...
def plot_Surface_Potentials(
    ...
    if survey == "Dipole-Pole" or survey == "Pole-Pole":
        ...
        xytextM = (M + 0.5, max(min(VM, ylim.max()), ylim.min()) + 1)
        xytextN = (N + 0.5, max(min(VN, ylim.max()), ylim.min()) + 1)
        ax[0].annotate("%2.1e" % (VM), xy=xytextM, xytext=xytextM, fontsize=labelsize)
...
def plate_app():
    app = widgetify(
        ...
        dx=FloatSlider(
            min=1.0, max=80.0, step=1.0, value=10.0, continuous_update=False
        ),
        dz=FloatSlider(
            min=1.0, max=40.0, step=1.0, value=10.0, continuous_update=False
        ),
        xc=FloatSlider(
