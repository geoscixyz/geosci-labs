...
def get_Setup(XYZ, sig0, sig1, R, E0, ax, label, colorsphere):
    ...
    if R > 0:
        ...
        if label:
            ax.annotate(
                ("$\sigma_1$={:3.3f} mS/m".format(sig1 * 10.0 ** (3.0))),
                xy=(0.0, -R / 2.0),
                xycoords="data",
                xytext=(0.0, -R / 2.0),
                textcoords="data",
            ...
            ax.annotate(
                ("$\sigma_0$= {:3.3f} mS/m".format(sig0 * 10.0 ** (3.0))),
                xy=(0.0, -1.5 * R),
                xycoords="data",
                xytext=(0.0, -1.5 * R),
                textcoords="data",
            ...
            ax.annotate(
                ("$R$ = {:1.0f} m".format(R)),
                xy=(R / 4.0 + (xr[1] - xr[0]), R / 4.0),
                xycoords="data",
                xytext=(R / 4.0 + (xr[1] - xr[0]), R / 4.0),
                textcoords="data",
                fontsize=14.0,
            )
            ...
            ax.set_xticklabels([])
            ax.set_yticklabels([])
            ax.text(-1.0, -np.sqrt(R) / 2.0 - 10.0, "$\sigma_1$", fontsize=14)
...
def get_Potential(XYZ, sig0, sig1, R, E0):
    ...
    Vt[ind0] = (
        -E0 * x[ind0] * (1.0 - sig_cur * R ** 3.0 / r_cur[ind0] ** 3.0)
    )  # total potential outside the sphere
    Vt[ind1] = -E0 * x[ind1] * 3.0 * sig0 / (sig1 + 2.0 * sig0)  # inside the sphere
...
def get_ElectricField(XYZ, sig0, sig1, R, E0):
    ...
    Et[ind0, 0] = E0 + E0 * R ** 3.0 / (r_cur[ind0] ** 5.0) * sigf(sig0, sig1) * (
        2.0 * x[ind0] ** 2.0 - y[ind0] ** 2.0 - z[ind0] ** 2.0
    )
    Et[ind0, 1] = (
        E0
        * R ** 3.0
    ...
    Et[ind1, 0] = 3.0 * sig0 / (sig1 + 2.0 * sig0) * E0
    Et[ind1, 1] = 0.0
    Et[ind1, 2] = 0.0
...
def Plot_Total_ElectricField(XYZ, sig0, sig1, R, E0, ax):
    ...
    EtAmp = np.sqrt(Et[:, 0] ** 2 + Et[:, 1] ** 2 + Et[:, 2] ** 2).reshape(
        xr.size, yr.size
    )
    ...
    cb.ax.tick_params(labelsize=ftsize_axis)
    ax.streamplot(
        xr, yr, EtXr, EtYr, color="gray", linewidth=2.0, density=0.75
...
def Plot_Secondary_ElectricField(XYZ, sig0, sig1, R, E0, ax):
    ...
    EsAmp = np.sqrt(Es[:, 0] ** 2 + Es[:, 1] ** 2 + Es[:, 2] ** 2).reshape(
        xr.size, yr.size
    )
    ...
    cb.ax.tick_params(labelsize=ftsize_axis)
    ax.streamplot(
        xr, yr, EsXr, EsYr, color="gray", linewidth=2.0, density=0.75
    )  # , angles='xy', scale_units='xy', scale=0.05)
    ax.plot(
        xcirc,
        np.sqrt(R ** 2 - xcirc ** 2),
...
def Plot_Total_Currents(XYZ, sig0, sig1, R, E0, ax):
    ...
    JtAmp = np.sqrt(Jt[:, 0] ** 2 + Jt[:, 1] ** 2 + Jt[:, 2] ** 2).reshape(
        xr.size, yr.size
    )
    ...
    cb.ax.tick_params(labelsize=ftsize_axis)
    ax.streamplot(
        xr, yr, JtXr, JtYr, color="gray", linewidth=2.0, density=0.75
...
def Plot_Secondary_Currents(XYZ, sig0, sig1, R, E0, ax):
    ...
    JsAmp = np.sqrt(Js[:, 1] ** 2 + Js[:, 0] ** 2 + Jt[:, 2] ** 2).reshape(
        xr.size, yr.size
    )
    ...
    cb.ax.tick_params(labelsize=ftsize_axis)
    ax.streamplot(
        xr, yr, JsXr, JsYr, color="gray", linewidth=2.0, density=0.75
    )  # , angles='xy', scale_units='xy', scale=1)
...
def MN_Potential_total(sig0, sig1, R, E0, start, end, nbmp, mn):
    ...
    D = np.sqrt((start[0] - end[0]) ** 2.0 + (start[1] - end[1]) ** 2.0)
    ...
    for n in range(0, len(EL), 2):
        EL[n, 0] = MP[int(n / 2), 0] - ((end[0] - start[0]) / D) * mn / 2.0
        EL[n + 1, 0] = MP[int(n / 2), 0] + ((end[0] - start[0]) / D) * mn / 2.0
        EL[n, 1] = MP[int(n / 2), 1] - ((end[1] - start[1]) / D) * mn / 2.0
        EL[n + 1, 1] = MP[int(n / 2), 1] + ((end[1] - start[1]) / D) * mn / 2.0
    ...
    dVtMP = np.zeros(
        nbmp
    )  # Diffence (d-) of Total Potential (Vt-) at each dipole (-MP)
    dVtMPn = np.zeros(
        nbmp
    )  # Diffence (d-) of Total Potential (Vt-) at each dipole (-MP) normalized for the mn spacing (n)
    dVsMP = np.zeros(
        nbmp
    )  # Diffence (d-) of Secondaty Potential (Vt-) at each dipole (-MP)
    dVsMPn = np.zeros(
        nbmp
    ...
    for m in range(0, 2 * nbmp):
        if r(EL[m, 0], EL[m, 1], 0) > R:
            VtEL[m] = (
                -E0
                * EL[m, 0]
                * (1.0 - sigf(sig0, sig1) * R ** 3.0 / r(EL[m, 0], EL[m, 1], 0) ** 3.0)
            )
        else:
            VtEL[m] = -E0 * EL[m, 0] * 3.0 * sig0 / (sig1 + 2.0 * sig0)
...
def two_configurations_comparison(
    XYZ,
    sig0,
    sig1,
    sig2,
    ...
    MP0, EL0, VtdMP0, VtdMPn0, VsdMP0, VsdMPn0 = MN_Potential_total(
        sig0, sig1, R0, E0, start, end, nb_dipole, electrode_spacing
    )
    MP1, EL1, VtdMP1, VtdMPn1, VsdMP1, VsdMPn1 = MN_Potential_total(
        sig0, sig2, R1, E0, start, end, nb_dipole, electrode_spacing
    )
    ...
    ax3.legend(
        [Dip_Midpoint0, Electrodes0], ["Dipole Midpoint", "Electrodes"], scatterpoints=1
    )
    ...
    ax4.legend(
        [Dip_Midpoint1, Electrodes1], ["Dipole Midpoint", "Electrodes"], scatterpoints=1
    )
...
def interact_conductiveSphere(
    R, log_sig0, log_sig1, Figure1a, Figure1b, Figure2a, Figure2b
):

    sig0, sig1 = conductivity_log_wrapper(log_sig0, log_sig1)
    E0 = 1.0  # inducing field strength in V/m
    n = 100  # level of discretisation
    xr = np.linspace(-200.0, 200.0, n)  # X-axis discretization
    ...
    if Figure1a == "Configuration":
        ...
        print(
            "While figure1 is plotting Configuration, figure2 plots the primary field"
        )
...
if __name__ == "__main__":
    sig0 = -3.0  # conductivity of the wholespace
    sig1 = -1.0  # conductivity of the sphere
    sig0, sig1 = conductivity_log_wrapper(sig0, sig1)
    R = 50.0  # radius of the sphere
    E0 = 1.0  # inducing field strength
    n = 100  # level of discretisation
    xr = np.linspace(-2.0 * R, 2.0 * R, n)  # X-axis discretization
    yr = xr.copy()  # Y-axis discretization
    zr = np.r_[0]  # identical to saying `zr = np.array([0])`
    XYZ = ndgrid(xr, yr, zr)  # Space Definition
