...
def run_simulation(
    ...
    hz = Utils.meshTensor([(cs, npadz, -1.3), (cs/2., ncz), (cs, 5, 2)])
    mesh = Mesh.TensorMesh([hx, hy, hz], x0=['C', 'C', -hz[:int(npadz+ncz/2)].sum()])
    ...
    rxList = EM.TDEM.Rx.Point_dbdt(np.r_[10., 0., 30.], times, orientation="z")
    if src_type == "VMD":
        src = EM.TDEM.Src.CircularLoop([rxList], loc=np.r_[0., 0., 30.], orientation="Z", waveform=EM.TDEM.Src.StepOffWaveform(), radius=13.)
    elif src_type == "HMD":
        src = EM.TDEM.Src.MagDipole([rxList], loc=np.r_[0., 0., 30.], orientation="X", waveform=EM.TDEM.Src.StepOffWaveform())
    SrcList = [src]
    ...
    prb.pair(survey)
    prb.Solver = Pardiso
    prb.timeSteps = [(1e-06, 5), (5e-06, 5), (1e-05, 10), (5e-05, 10), (1e-4, 15), (5e-4, 16)]
...
class PlotTDEM(object):
    ...
    def show_3d_survey_geometry(self, elev, azim):
        ...
        ax.plot3D(np.r_[0, 0], np.r_[0, 0], np.r_[1, 1]*30., 'ro', ms=5)
        ax.legend(("Tx",), loc=1)
    ...
    def getSlices(self, mesh, vec, itime, normal="Z", loc=0., isz=False, isy=False):
    ...
    def plot_electric_currents(self, itime):
        ...
        exz, xz = self.getSlices(self.mesh, self.J, itime, normal="Y", loc= 0., isy=True)
